...
export default function Cart() {
  ...
  return (
    <div>
        ...
        <button className='cart_btn' onClick={() => dispatch(changeName())}>닉네임보이기</button>
          ...
          state.cart.map((item,i) => {
            return(
              <tr key={i}>
                ...
                <td >
                  <button className='sub_btn' onClick={()=>dispatch(addCount(state.cart[i].id))}>+</button>
                  <button className='sub_btn' onClick={()=>dispatch(subCount(state.cart[i].id))}>-</button>
                  </td>
                  <td>
                  <button className='sub_btn delete_btn' onClick={()=>dispatch(deleteItem(state.cart[i].id))}>삭제</button>
                </td>
              </tr>
            )
          })
